...
local AUTOSAVE_INTERVAL = 6       -- periodic autosave (not currently auto-called but available)
...
-- helper to send two Roblox notifications (Welcome, Autosave/Status)
local function sendNeoZNotifications(welcomeText, autosaveText, duration)
...
-- ===== Modern Glow Notifications (kept available) =====
...
-- keep this function available (used elsewhere) but notifications are unified via Roblox SetCore by default
...
local function tryLoad()
    ...
    if not content then
        -- send unified Roblox notifications: Welcome, Autosave Not Found
        sendNeoZNotifications(welcomeText, "⚠️ AutoSave Not Found / Unsupported Executor", 3.5)
        setToggleAppearance()
        return
    end
    ...
    if ok and type(state) == "table" then
        applyState(state)
        -- send unified Roblox notifications: Welcome, Autosave Loaded
        sendNeoZNotifications(welcomeText, "✅ AutoSave Loaded Successfully", 3.5)
    else
        -- send unified Roblox notifications: Welcome, AutoSave Corrupt
        sendNeoZNotifications(welcomeText, "⚠️ AutoSave Corrupt", 3.5)
    end
end
pcall(tryLoad)
